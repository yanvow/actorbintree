...
trait TestKitBase {
   ...
   */
  def remainingOrDefault = remainingOr(testKitSettings.SingleExpectDefaultTimeout)
   ...
   * block.
   *
   * Note that the timeout is scaled using Duration,
   ...
   *
   * Note that the timeout is scaled using Duration, which uses the
   ...
   */
  def expectMsg[T](max: FiniteDuration, obj: T): T = expectMsg_internal(max, obj)
   ...
   * @return the received object
   */
  def expectMsg[T](max: FiniteDuration, hint: String, obj: T): T = expectMsg_internal(max, obj, Some(hint))
  ...
  def expectMsgType[T](max: FiniteDuration)(implicit t: ClassTag[T]): T =
    expectMsgClass_internal(max, t.runtimeClass.asInstanceOf[Class[T]])
   ...
   * @return the received object
   */
  def expectMsgClass[C](max: FiniteDuration, c: Class[C]): C = expectMsgClass_internal(max, c)
   ...
   * @return the received object
   */
  def expectMsgAnyOf[T](max: FiniteDuration, obj: T*): T = expectMsgAnyOf_internal(max, obj: _*)
  ...
  def expectMsgAnyClassOf[C](max: FiniteDuration, obj: Class[_ <: C]*): C =
    expectMsgAnyClassOf_internal(max, obj: _*)
   ...
   * </pre>
   */
  def expectMsgAllOf[T](max: FiniteDuration, obj: T*): immutable.Seq[T] = expectMsgAllOf_internal(max, obj: _*)
  ...
  def expectMsgAllClassOf[T](max: FiniteDuration, obj: Class[_ <: T]*): immutable.Seq[T] =
    internalExpectMsgAllClassOf(max, obj: _*)
  ...
  def expectMsgAllConformingOf[T](max: FiniteDuration, obj: Class[_ <: T]*): immutable.Seq[T] =
    internalExpectMsgAllConformingOf(max, obj: _*)
  ...
  def expectNoMessage(): Unit =
    expectNoMsg_internal(testKitSettings.ExpectNoMessageDefaultTimeout)
   ...
   */
  def receiveN(n: Int, max: FiniteDuration): immutable.Seq[AnyRef] = receiveN_internal(n, max)
 ...
 * are scaled using their Duration method, which uses the
...
class TestKitSettings(val config: Config) extends Extension {
  ...
  val SingleExpectDefaultTimeout: FiniteDuration = 3.seconds
  val ExpectNoMessageDefaultTimeout: FiniteDuration = 100.milliseconds
  val TestEventFilterLeeway: FiniteDuration = 3.seconds
